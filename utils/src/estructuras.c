...
void eliminar_instrucciones(t_list *instrucciones) {
	list_destroy_and_destroy_elements(instrucciones, eliminar_instruccion);
...
t_pcb *crear_pcb(uint32_t id, uint32_t tamanio_proceso, uint32_t prioridad) {
	...
	pcb->recurso_pendiente = string_new();
...
void eliminar_pcb(t_pcb *pcb) {
	...
	dictionary_destroy_and_destroy_elements(pcb->recursos_asignados, free);
	free(pcb->recurso_pendiente);
	free(pcb);
}
